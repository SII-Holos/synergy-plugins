...
3. Add or update `plugins/<plugin-id>.json` in this repository. Registry v2 records `apiVersion` and `compatibility.synergy` for every version, matching the packaged `plugin.json`. Each version must include `signature.algorithm: "ed25519"` and `signature.signer` from the `.sig` file.
...
The entry must include API and host compatibility, a natural-language feature summary separate from its access summary, a `sha256-...` integrity string, a signature metadata URL, and the signer public key. CI verifies the downloaded artifact hash, required package files, manifest identity (`id` for API3/API4, `name` for historical API2 artifacts) and version, signature payload hashes, and the Ed25519 signature using the registry-reviewed signer.

Only non-yanked Plugin API 4 versions are selected as default installation candidates. Historical API2/API3 releases remain visible for provenance but are never selected for a new install.
